...
> already in hand** (three in clean square type). Plan: OCR the square editions, structure
> them into the native three-part / *klalim* schema, ingest — turning 287 dead references
> live. **Cost:** a lean single-edition path is a few hundred dollars; the full ensemble
> adds a one-time, reusable harness. **First step:** a proofed *Klalei HaGemara*
> Aleph-section pilot.
...
1. **Pilot** the *Klalei HaGemara* Aleph section (OCR'd from Berlin, link-readiness-checked)
   to de-risk the schema end-to-end.
2. **Then the full work**, attaching each printing as its own version.
3. **Coordinate ingest** with Sefaria (**hello@sefaria.org**).
