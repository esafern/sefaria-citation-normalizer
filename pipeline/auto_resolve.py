...
cands = json.load(open(os.path.join(HERE, "v3_candidates.json")))
...
cache_path = os.path.join(HERE, "name_cache.json")
cache = json.load(open(cache_path)) if os.path.exists(cache_path) else {}
_lock = threading.Lock()
WORKERS = 3            # gentle
DELAY = 0.15          # per /api/name call
...
json.dump(resolved, open(os.path.join(HERE, "auto_resolved.json"), "w"), ensure_ascii=False, indent=1)
json.dump(sorted(unresolved), open(os.path.join(HERE, "unresolved_for_labeling.json"), "w"), ensure_ascii=False, indent=1)
